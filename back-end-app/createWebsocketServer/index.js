...
const startServer = (data, homeWindow) => {
  try {
    ...
    process = runProject(data.proxyCommand, data.proxyFolderPath, homeWindow)
    ...
    wss.on("connection", function connection(ws) {
      ...
      if (data.proxyPort) {
        let backendWs
        while (!backendWs || backendWs.readyState !== WebSocket.CONNECTING) {
          backendWs = new WebSocket(`ws://localhost:${data.proxyPort}`,{headers: {'origin':"http://localhost:8888"}})
          setTimeout(() => {
            stopServer()
            return false
          }, 60 * 1000 * 10)
        ...
        backendWs.on("message", (backendMsg) => {
          ...
          homeWindow.webContents.send("wssReceivedMsg", backendMsg)
        })
...
const connectServer = (serverData, homeWindow) => {
  try {
    ...
    const ws = new WebSocket(serverData.url, { headers: { Cookie: cookie } })
    wsMessageConnection = ws
    ws.onopen = (event) => {
    ...
    ws.onmessage = (data) => {
      ...
      homeWindow.webContents.send("wssReceivedMsg", data.data)
    }
  } catch (error) {
